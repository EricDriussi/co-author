...
pub fn init_repo(path: &PathBuf) -> Result<Repository, Box<dyn std::error::Error>> {
...
pub fn set_user_and_email(conf: &mut Config) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn create_and_add_file_to_git_tree(repo: &Repository, file_name: &str) -> Result<(), Box<dyn std::error::Error>> {
